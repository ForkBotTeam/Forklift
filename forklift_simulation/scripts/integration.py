...
def QR_Data(data,data_saved,centers) : # Append new data and centroids of QR codes detected
    if Data not in data_saved:
        print('New Data: ',Data)
        path=os.path.join('images','detect',f'{uuid.uuid1()}.jpg')
        data_saved.append(Data)
        centers.append([(np.mean(pts[0],axis=0)[0],np.mean(pts[0],axis=0)[1]),Data]) # This list contains the centroids of QR codes detected <-- This is what you need
        cv2.imwrite(path,cframe)
def QR_Centroid_Update(data,data_saved,centers,pts): # Update the centroids of QR codes detected
    if data in data_saved:
        index=data_saved.index(data)
        centers[index][0]=(np.mean(pts[0],axis=0)[0],np.mean(pts[0],axis=0)[1])
    return centers 
...
if __name__== "__main__":
    ...
    qr_centers=[]
    k= 0
    while k!=27: # quit program when 'esc' key is pressed
        ...
        pts.reshape((-1,1,2))
        qr_centers = QR_Centroid_Update(Data,data_saved,qr_centers,pts) # Update the centroids of QR codes detected
        ...
        QR_Data(Data,data_saved,qr_centers) # Append new data and centroids of QR codes detected
        ...
        for qr_center in qr_centers:
            far = how_far(depth, qr_center[0][0], qr_center[0][1])
            cv2.putText(frame, str(far),(qr_center[0][0],qr_center[0][1]),fontFace= 2, fontScale= 0.75, color= (20,20,150),thickness= 1)
